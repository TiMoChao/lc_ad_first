...
class phpzip{
     ...
     * @access public
     */
    function add_cachefile($file, $name, $time = 0){
        $name     = str_replace('\\', '/', $name);
		...
		if (!$fp = @fopen($file,'r')) {
			...
			//$zip->add_file($contents,$arrGPdoDB['db_name'].'_'.$date.'_'.$i.'.sql');
			//$i++;
			//$sql = $head.$i."\n-- \n-- DATABASE : ".$arrGPdoDB['db_name']."\n-- \n\n-- ---------------------------------\n";
            //@fclose($fp);
		}

		//php还没有crc32_file函数，所以这里文件大于8M就会发生程序内存溢出错误
