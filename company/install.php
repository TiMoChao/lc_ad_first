...
include_once(dirname(__FILE__).'/class/company.class.php');
...
include($strFilename);
$arrGMeta['company']['name'] = '企业黄页';
$arrGMeta['company']['cache'] = 1;
if(!is_array($arrMType)||empty($arrMType)){
	$arrGMeta['company']['admin'] = array(
									array(
										  'href'=>'../company/admin/category.php',
										  'name'=>'企业黄页分类',),
									array(
										  'href'=>'../company/admin/index.php',
										  'name'=>'企业黄页管理',),
									);
}else{
	$arrGMeta['company']['admin'] = array(
...
$arrGMeta['company']['meta'] = array(
									  'Title' => $arrGMeta['company']['name'],
									  'Description' => $arrGMeta['company']['name'],
									  'Keywords' => $arrGMeta['company']['name'],
									);
...
//sql语句
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."company` (
...
) ENGINE=MyISAM {$extend} COMMENT='企业黄页表' ;";

if(!is_array($arrMType)||empty($arrMType)){
	$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."company_type` (
	...
	) ENGINE=MyISAM  {$extend} COMMENT='企业黄页分类表' ;";
}
...
if(empty($arrModule)){
	if(!is_array($arrMType)||empty($arrMType)){
		check::AlertExit('数据库分类企业黄页系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
	}else check::AlertExit('配置文件分类企业黄页系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>