...
include($strFilename);
$arrGMeta['ask']['name'] = '问题解答';
$arrGMeta['ask']['cache'] = 1;
if(!is_array($arrMType)||empty($arrMType)){
	$arrGMeta['ask']['admin'] = array(
									array(
										  'href'=>'../ask/admin/category.php',
										  'name'=>'问题解答分类',),
									array(
										  'href'=>'../ask/admin/index.php',
										  'name'=>'问题解答管理',),
									);
}else{
	$arrGMeta['ask']['admin'] = array(
...
$arrGMeta['ask']['meta'] = array(
									  'Title' => $arrGMeta['ask']['name'],
									  'Description' => $arrGMeta['ask']['name'],
									  'Keywords' => $arrGMeta['ask']['name'],
									);
...
//sql语句
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."ask` (
...
) ENGINE=MyISAM {$extend} CHARACTER SET utf8 COLLATE utf8_general_ci COMMENT='问题答疑表' ;";

if(!is_array($arrMType)||empty($arrMType)){
	$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."ask_type` (
	...
	) ENGINE=MyISAM  {$extend} CHARACTER SET utf8 COLLATE utf8_general_ci COMMENT='行业资讯分类表' ;";
}
...
if(empty($arrModule)){
	if(!is_array($arrMType)||empty($arrMType)){
		check::AlertExit('数据库分类问题答疑系统安装成功',"$strWEB_ROOT_pre/$strWEBADMIN_ROOT/");
	}else check::AlertExit('配置文件分类问题答疑系统安装成功',"$strWEB_ROOT_pre/$strWEBADMIN_ROOT/");
}
?>