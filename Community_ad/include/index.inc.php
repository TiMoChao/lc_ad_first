...
foreach($arrMType as $k=>$v){
	...
	if(is_array($v)) {
		...
		$arrTopCommunity_ad[$k]['type_title'] = $v;
		$arrTopCommunity_ad[$k]['type_id'] = $k;
	}
	$arrTopCommunity_ad[$k]['datas'] = $objWebInit->getInfoList("where pass=1 and type_id=".$arrTopCommunity_ad[$k]['type_id'],"  ORDER BY recommendflag DESC,clicktimes DESC,submit_date DESC", 0, 12,true,'',false);
}

// 输出到模板
$arrMOutput["smarty_assign"]['arrTopCommunity_ad'] = $arrTopCommunity_ad;
