...
//		vZoom.loadingAreaImage.style.height = '0px';
	...
	add : function(element, largerImageURL) {
		...
		if (!vZoom.loadingAreaImage) {
			...
			vZoom.loadingAreaImage.src = '/plug-in/vzoom/vzoom.gif';
		}
...
function change_product_photo(arg_photo_number) {
	var src_part,dest_part;
	src_part = document.getElementById("alternate_photo_" + previous_selected_photo);
	dest_part = document.getElementById("alternate_photo_" + arg_photo_number);

	//HIDE all previously open spans (if they exist)
	if (src_part) {
		src_part.style.border = "3px solid #ccc";
	}
	
	//SHOW the selected SPAN tag + GROUP BY span tag (if they exist)
	if (dest_part) {
		dest_part.style.border = "3px solid blue";
		//dest_part.src = dest_part.src.replace('/s/','/b/');
		document.getElementById("vzoom_photo").src = dest_part.src;
	}
