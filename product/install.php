...
include($strFilename);
$arrGMeta['product']['name'] = '商展信息';
$arrGMeta['product']['cache'] = 1;
if(!is_array($arrMType)||empty($arrMType)){
	$arrGMeta['product']['admin'] = array(
									array(
										  'href'=>'../product/admin/category.php',
										  'name'=>'商展信息分类',),
									array(
										  'href'=>'../product/admin/index.php',
										  'name'=>'商展信息管理',),
									);
}else{
	$arrGMeta['product']['admin'] = array(
...
$arrGMeta['product']['meta'] = array(
									  'Title' => $arrGMeta['product']['name'],
									  'Description' => $arrGMeta['product']['name'],
									  'Keywords' => $arrGMeta['product']['name'],
									);
...
//sql语句
$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."product` (
...
) ENGINE=MyISAM {$extend} CHARACTER SET utf8 COLLATE utf8_general_ci COMMENT='商展信息表' ;";

if(!is_array($arrMType)||empty($arrMType)){
	$sql[] = "CREATE TABLE IF NOT EXISTS `".$arrGPdoDB['db_tablepre']."product_type` (
	...
	) ENGINE=MyISAM  {$extend} CHARACTER SET utf8 COLLATE utf8_general_ci COMMENT='商展信息分类表' ;";
}
...
if(empty($arrModule)){
	if(!is_array($arrMType)||empty($arrMType)){
		check::AlertExit('数据库分类商展信息系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
	}else check::AlertExit('配置文件分类商展信息系统安装成功',"$strWEB_ROOT_pre$strWEBADMIN_ROOT/");
}
?>